...
const (
	...
	metricsRPLengthLabel  = "request_pool_length"
	metricsRPSizeLabel    = "request_pool_size"
)
...
// RequestTimeoutHandler defines the methods called by request timeout timers
// created by time.AfterFunc. This interface is implemented by the bft.Controller.
...
func (rp *Pool) ChangeTimeouts(th RequestTimeoutHandler, options PoolOptions) {
	...
	if !rp.stopped {
		rp.diag.L().Errorf("Trying to change timeouts but the pool is not stopped")
		return
	}
	...
	rp.diag.L().Debugf("Changed pool timeouts")
}
...
func (rp *Pool) Submit(request []byte) error {
	...
	if alreadyExists {
		rp.diag.L().Debugf("request %s already exists in the pool", reqInfo)
	...
	if rp.stopped {
		rp.diag.L().Debugf("pool stopped, submitting with a stopped timer, request: %s", reqInfo)
		to.Stop()
	}
	reqItem := &requestItem{
		request: reqCopy,
	...
	if len(rp.existMap) != rp.fifo.Len() {
		rp.diag.L().Panicf("RequestPool map and list are of different length: map=%d, list=%d", len(rp.existMap), rp.fifo.Len())
	}

	rp.diag.L().Debugf("Request %s submitted; started a timeout: %s", reqInfo, rp.options.ForwardTimeout)
...
func (rp *Pool) NextRequests(maxCount int, maxSizeBytes uint64, check bool) (batch [][]byte, full bool) {
	...
	for i := 0; i < count; i++ {
		...
		if totalSize+reqLen > maxSizeBytes {
			rp.diag.L().Debugf("Returning batch of %d requests totalling %dB as it exceeds threshold of %dB",
	...
	full = fullS || fullC
	if len(batch) > 0 {
		rp.diag.L().Debugf("Returning batch of %d requests totalling %dB",
			len(batch), totalSize)
	}
	return batch, full
}
...
func (rp *Pool) Prune(predicate func([]byte) error) {
	...
	for i, req := range reqVec {
		...
		if remErr := rp.RemoveRequest(infoVec[i]); remErr != nil {
			rp.diag.L().Debugf("Failed to prune request: %s; predicate error: %s; remove error: %s", infoVec[i], err, remErr)
		} else {
			rp.diag.L().Debugf("Pruned request: %s; predicate error: %s", infoVec[i], err)
			numPruned++
		}
	}

	rp.diag.L().Debugf("Pruned %d requests", numPruned)
}
...
func (rp *Pool) RemoveRequest(requestInfo types.RequestInfo) error {
	...
	reqSize := len(element.Value.(*requestItem).request)
...
func (rp *Pool) deleteRequest(element *list.Element, requestInfo types.RequestInfo) {
	...
	rp.diag.L().Infof("Removed request %s from request pool", requestInfo)
	rp.semaphore.Release(1)
...
func (rp *Pool) StopTimers() {
	...
	rp.diag.L().Debugf("Stopped all timers: size=%d", len(rp.existMap))
}
...
func (rp *Pool) RestartTimers() {
	...
	rp.diag.L().Debugf("Restarted all timers: size=%d", len(rp.existMap))
}
...
func (rp *Pool) onRequestTO(request []byte, reqInfo types.RequestInfo) {
	...
	if !contains {
		rp.lock.Unlock()
		rp.diag.L().Debugf("Request %s no longer in pool", reqInfo)
		return
	}

	if rp.closed || rp.stopped {
		rp.lock.Unlock()
		rp.diag.L().Debugf("Pool stopped, will NOT start a leader-forwarding timeout")
		return
	}
	...
	rp.diag.L().Debugf("Request %s; started a leader-forwarding timeout: %s", reqInfo, rp.options.ComplainTimeout)

	rp.lock.Unlock()

	// may take time, in case Comm channel to leader is full; hence w/o the lock.
	rp.diag.L().Debugf("Request %s timeout expired, going to send to leader", reqInfo)
...
func (rp *Pool) onLeaderFwdRequestTO(request []byte, reqInfo types.RequestInfo) {
	...
	if rp.closed || rp.stopped {
		rp.lock.Unlock()
		rp.diag.L().Debugf("Pool stopped, will NOT start auto-remove timeout")
		return
	}
	...
	rp.diag.L().Debugf("Request %s; started auto-remove timeout: %s", reqInfo, rp.options.AutoRemoveTimeout)

	rp.lock.Unlock()

	// may take time, in case Comm channel is full; hence w/o the lock.
	rp.diag.L().Debugf("Request %s leader-forwarding timeout expired, going to complain on leader", reqInfo)
	rp.timeoutHandler.OnLeaderFwdRequestTimeout(request, reqInfo)
}

// called by the goroutine spawned by time.AfterFunc
func (rp *Pool) onAutoRemoveTO(reqInfo types.RequestInfo) {
	rp.diag.L().Debugf("Request %s auto-remove timeout expired, going to remove from pool", reqInfo)
	if err := rp.RemoveRequest(reqInfo); err != nil {
		rp.diag.L().Errorf("Removal of request %s failed; error: %s", reqInfo, err)
		return
	}
