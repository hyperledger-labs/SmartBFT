...
func NewChain(id uint64, in Ingress, out Egress, diag smart.Diagnostics, opts NetworkOptions, testDir string) *Chain {
	deliverChan := make(chan *Block)
	node := NewNode(id, in, out, deliverChan, diag, opts, testDir)
	return &Chain{
		node:        node,
